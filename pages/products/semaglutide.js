...
export default function Semaglutide() {
  ...
  const finalPrice =
    purchaseType === "subscription" ? subscriptionPrice : current.price;
  ...
  const bundleSubtotal = selectedBundleItems.reduce(
  ...
  const alsoBoughtSubtotal = selectedAlsoBoughtItems.reduce(
    (sum, item) => sum + item.price,
    0
  );
  ...
  const buildDiscountedItems = (items, discountRate, label) => {
    ...
    return items.map((item) => ({
      ...
      name: item.name,
      variant: `${label}${
        discountRate > 0 ? ` • ${Math.round(discountRate * 100)}% savings` : ""
      }`,
  ...
  const buildCartItems = () => {
    const items = [
      buildMainItem(),
      ...buildDiscountedItems(
        selectedBundleItems,
        bundleDiscountRate,
        "Bundle Add-On"
      ),
      ...buildDiscountedItems(
        selectedAlsoBoughtItems,
        alsoBoughtDiscountRate,
        "People Also Bought"
      ),
    ];

    if (bumpSelected) {
      items.push(buildBumpItem());
    }
  ...
  return (
    <div className="semaglutide-page">
      <div className="container">
        ...
        <section className="product-page premium-product-page">
          ...
          <div
            className={`product-image-wrap premium-image-wrap ${
              showInfo ? "glow-active" : ""
            }`}
          >
            <div
              className={`product-image-stack premium-image-stack ${
                animateAdd ? "cart-added-pulse" : ""
              }`}
            >
              <img
                src={current.image}
                ...
                <div className="frequency-options">
                  <button
                    ...
                    className={`frequency-pill ${
                      subscriptionFrequency === "monthly" ? "active" : ""
                    }`}
                  ...
                  <button
                    type="button"
                    className={`frequency-pill ${
                      subscriptionFrequency === "biweekly" ? "active" : ""
                    }`}
